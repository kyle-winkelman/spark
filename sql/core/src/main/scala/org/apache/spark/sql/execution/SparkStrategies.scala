...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  /**
   * Strategy to convert [[TransformWithStateInPandas]] logical operator to physical operator in
   * streaming plans.
   */
  ...
  object BasicOperators extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case f @ logical.FlatMapCoGroupsInPandas(_, func, output, children) =>
        execution.python.FlatMapCoGroupsInPandasExec(
          f.groups,
          func,
          output,
          children.map(planLater)) :: Nil
      case f @ logical.FlatMapCoGroupsInArrow(_, func, output, children) =>
      ...
      case t @ TransformWithStateInPandas(
            ...
            _,
            initialStateSchema) =>
        TransformWithStateInPandasExec.generateSparkPlanForBatchQueries(
          func,
          t.leftAttributes,
          outputAttrs,
          outputMode,
