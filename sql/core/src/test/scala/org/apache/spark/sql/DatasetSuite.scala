...
class DatasetSuite extends QueryTest with SharedSparkSession with AdaptiveSparkPlanHelper {
  ...
  test("checkAnswer should compare map correctly") {
    ...
    checkAnswer(data.toDF(), Seq(Row(1, "2", Map(2 -> 1, 1 -> 2))))
  }
  ...
  test("coalesce, repartition") {
    ...
    checkDatasetUnorderly(ds.repartition(10), data: _*)

    assert(ds.coalesce(1).rdd.partitions.length == 1)
    checkDatasetUnorderly(ds.coalesce(1), data: _*)
  }
  ...
  test("as map of case class - reorder fields by name") {
    val df =
      spark.range(3).select(map(lit(1), struct($"id".cast("int").as("b"), lit("a").as("a"))))
  ...
  test("map and group by with class data") {
    ...
    val ds: Dataset[(ClassData, Long)] = Seq(ClassData("one", 1), ClassData("two", 2))
      .toDS()
      .map(c => ClassData(c.a, c.b + 1))
      .groupByKey(p => p)
      .count()

    checkDatasetUnorderly(ds, (ClassData("one", 2), 1L), (ClassData("two", 3), 1L))
  }

  test("SPARK-45896: seq of option of seq") {
    val ds = Seq(DataSeqOptSeq(Seq(Some(Seq(0))))).toDS()
    checkDataset(ds, DataSeqOptSeq(Seq(Some(List(0)))))
  }
  ...
  test("select 2, primitive and class") {
    val ds = Seq(("a", 1), ("b", 2), ("c", 3)).toDS()
    checkDataset(
      ds.select(expr("_1").as[String], expr("named_struct('a', _1, 'b', _2)").as[ClassData]),
      ("a", ClassData("a", 1)),
      ("b", ClassData("b", 2)),
      ("c", ClassData("c", 3)))
  }
  ...
  test("select 2, primitive and class, fields reordered") {
    val ds = Seq(("a", 1), ("b", 2), ("c", 3)).toDS()
    checkDataset(
      ds.select(expr("_1").as[String], expr("named_struct('b', _2, 'a', _1)").as[ClassData]),
      ("a", ClassData("a", 1)),
      ("b", ClassData("b", 2)),
      ("c", ClassData("c", 3)))
  }
  ...
  test("REGEX column specification") {
    ...
    withSQLConf(SQLConf.SUPPORT_QUOTED_REGEX_COLUMN_NAME.key -> "false") {
      checkError(
        ...
        context = ExpectedContext(fragment = "`(_1)?+.+`", start = 0, stop = 9))
      ...
      checkError(
        ...
        context = ExpectedContext(fragment = "`(_1|_2)`", start = 0, stop = 8))
    ...
    withSQLConf(SQLConf.SUPPORT_QUOTED_REGEX_COLUMN_NAME.key -> "true") {
      checkDataset(ds.select(ds.col("_2")).as[Int], 1, 2, 3)

      checkDataset(ds.select(ds.colRegex("`(_1)?+.+`")).as[Int], 1, 2, 3)
      ...
      checkDataset(ds.select(expr("`(_1)?+.+`").as[Int]), 1, 2, 3)
  ...
  test("filter and then select") {
    val ds = Seq(("a", 1), ("b", 2), ("c", 3)).toDS()
    checkDataset(ds.filter(_._1 == "b").select(expr("_1").as[String]), "b")
  }
  ...
  test("joinWith tuple with primitive, expression") {
    ...
    checkDataset(joined, (1, ("a", 1)), (1, ("a", 1)), (2, ("b", 2)))
  }
  ...
  test("joinWith join types") {
    ...
    def checkJoinWithJoinType(joinType: String): Unit = {
      ...
      checkError(
        exception =
          intercept[AnalysisException](ds1.joinWith(ds2, $"a.value" === $"b.value", joinType)),
  ...
  test("groupBy function, keys") {
    ...
    checkDatasetUnorderly(grouped.keys, (1, 1))
  }
  ...
  test("groupBy function, map") {
    ...
    checkDatasetUnorderly(aggregated, ("a", 30), ("b", 3), ("c", 1))
  }
  ...
  test("groupBy, flatMapSorted") {
    ...
    checkError(
      exception = intercept[AnalysisException] {
        grouped.flatMapSortedGroups($"*") { (g, iter) =>
  ...
  test("groupBy function, reduce") {
    ...
    checkDatasetUnorderly(aggregated, 3 -> "abcxyz", 5 -> "hello")
  }
  ...
  test("cogroup2") {
    ...
    checkDatasetUnorderly(cogrouped, 1 -> "a#", 2 -> "#q", 3 -> "abcfoo#w", 5 -> "hello#er")
  }
  ...
  test("cogroup with complex data") {
    ...
    checkDatasetUnorderly(cogrouped, 1 -> "a", 2 -> "bc", 3 -> "d")
  }
  ...
  test("cogroup with groupBy and sorted") {
    ...
    Seq(
      ...
      ("both desc", descOrder, descOrder, bothDescSortedExpected)).foreach {
      case (label, leftOrder, rightOrder, expected) =>
  ...
  test("SPARK-43781: cogroup two datasets derived from the same source") {
    val inputType = StructType(
      Array(StructField("id", LongType, false), StructField("type", StringType, false)))
    ...
    val fooGroups = input
      .filter("type = 'foo'")
      .groupBy("id")
      .as(ExpressionEncoder(keyType), ExpressionEncoder(inputType))
    val barGroups = input
      .filter("type = 'bar'")
      .groupBy("id")
      .as(ExpressionEncoder(keyType), ExpressionEncoder(inputType))

    val result = fooGroups
      .cogroup(barGroups) { case (row, iterator, iterator1) =>
        iterator.toSeq ++ iterator1.toSeq
      }(ExpressionEncoder(inputType))
      .collect()
    assert(result.length == 3)

    val result2 = fooGroups
      .cogroupSorted(barGroups)($"id")($"id") { case (row, iterator, iterator1) =>
        iterator.toSeq ++ iterator1.toSeq
      }(ExpressionEncoder(inputType))
      .collect()
  ...
  test("SPARK-48718: cogroup deserializer expr is resolved before dedup relation") {
    ...
    val cogrouped =
      lhsKV.cogroup(rhsKV)((a: Long, b: Iterator[Row], c: Iterator[Row]) => Iterator(0L))
  ...
  test("SPARK-34806: observation on datasets") {
    ...
    val observed_df = df
      ...
      .observe(unnamedObservation, avg($"id").cast("int").as("avg_val"))
  ...
  test("SPARK-45656: named observations with the same name on different datasets") {
    ...
    val observed_df1 = df1.observe(namedObservation1, count(lit(1)).as("count"))

    val namedObservation2 = Observation("named")
    val df2 = spark.range(100)
    val observed_df2 = df2.observe(namedObservation2, count(lit(1)).as("count"))
  ...
  test("sample with replacement") {
    ...
    checkDataset(data.sample(withReplacement = true, 0.05, seed = 13), 5, 10, 52, 73)
  }

  test("sample without replacement") {
    val n = 100
    val data = sparkContext.parallelize(1 to n, 2).toDS()
    checkDataset(data.sample(withReplacement = false, 0.05, seed = 13), 8, 37, 90)
  }
  ...
  test("sample fraction should not be negative with replacement") {
    ...
    checkDataset(data.sample(withReplacement = true, 1.05, seed = 13), 1, 2)
  }
  ...
  test("sample fraction should be on interval [0, 1] without replacement") {
    ...
    assert(
      errMsg1.contains("Sampling fraction (-0.1) must be on interval [0, 1] without replacement"))

    val errMsg2 = intercept[IllegalArgumentException] {
      data.sample(withReplacement = false, 1.1, 0)
    }.getMessage()
    assert(
      errMsg2.contains("Sampling fraction (1.1) must be on interval [0, 1] without replacement"))
  }
  ...
  test("Kryo encoder self join") {
    ...
    val ds = Seq(KryoData(1), KryoData(2)).toDS()
    assert(
      ds.joinWith(ds, lit(true), "cross").collect().toSet ==
        Set(
  ...
  test("runtime nullability check") {
    ...
    checkDataset(buildDataset(Row(Row("hello", 1))), NestedStruct(ClassData("hello", 1)))
  ...
  test("grouping key and grouped value has field with same name") {
    ...
    val aggregated = ds.groupByKey(d => ClassNullableData(d.a, null)).mapGroups { (key, values) =>
  ...
  test("SPARK-13540 Dataset of nested class defined in Scala object") {
    checkDataset(Seq(OuterObject.InnerClass("foo")).toDS(), OuterObject.InnerClass("foo"))
  }

  test("SPARK-14000: case class with tuple type field") {
    checkDataset(Seq(TupleClass((1, "a"))).toDS(), TupleClass((1, "a")))
  }
  ...
  test("createTempView") {
    ...
    val e = intercept[AnalysisException](dataset.createTempView("tempView"))
    ...
    withDatabase("test_db") {
      withTempView("tempView") {
        withSQLConf(SQLConf.ALLOW_TEMP_VIEW_CREATION_WITH_MULTIPLE_NAME_PARTS.key -> "false") {
          ...
          val e = intercept[AnalysisException](dataset.createTempView("test_db.tempView"))
          checkError(
            e,
  ...
  test("SPARK-15550 Dataset.show() should show inner nested products as rows") {
    val ds = Seq(NestedStruct(ClassData("foo", 1)), NestedStruct(ClassData("bar", 2))).toDS()
  ...
  test("SPARK-15112: EmbedDeserializerInFilter should not optimize plan that changes schema") {
  ...
  test("dropDuplicates") {
    ...
    checkDataset(ds.dropDuplicates("_1"), ("a", 1), ("b", 1))
    checkDataset(ds.dropDuplicates("_2"), ("a", 1), ("a", 2))
    checkDataset(ds.dropDuplicates("_1", "_2"), ("a", 1), ("a", 2), ("b", 1))
  }
  ...
  test("dropDuplicates: columns with same column name") {
    ...
    checkDataset(joined.dropDuplicates(), (1, 2), (1, 1), (2, 1), (2, 2))
  }
  ...
  test("SPARK-16995: flat mapping on Dataset containing a column created with lit/expr") {
    ...
    checkDataset(
      df.withColumn("b", lit(0))
        .as[ClassData]
        .groupByKey(_.a)
        .flatMapGroups { (_, _) => List[Int]() })
    checkDataset(
      df.withColumn("b", expr("0"))
        .as[ClassData]
        .groupByKey(_.a)
        .flatMapGroups { (_, _) => List[Int]() })
  }
  ...
  test("SPARK-18125: Spark generated code causes CompileException") {
    ...
    val grouped = ds
      .map(r => GroupedRoutes(r.src, r.dest, Seq(r)))
  ...
  Seq(true, false).foreach { eager =>
    Seq(true, false).foreach { reliable =>
      ...
      testCheckpointing("should preserve partitioning information") {
        ...
        agg.queryExecution.executedPlan
          .collectFirst {
  ...
  test("SPARK-17460: the sizeInBytes in Statistics shouldn't overflow to a negative number") {
    ...
    val ds = (0 to 10000)
      .map(i => (i, Seq((i, Seq((i, "This is really not that long of a string"))))))
      .toDS()
  ...
  test(
    "SPARK-18717: code generation works for both scala.collection.Map" +
  ...
  test("SPARK-18746: add implicit encoder for BigDecimal, date, timestamp") {
    ...
    assert(
      spark.range(1).map { x => new java.math.BigDecimal(1) }.head() ==
        new java.math.BigDecimal(1).setScale(18))

    assert(
      spark.range(1).map { x => scala.math.BigDecimal(1, 18) }.head() ==
        scala.math.BigDecimal(1, 18))

    assert(
      spark.range(1).map { x => java.sql.Date.valueOf("2016-12-12") }.head() ==
        java.sql.Date.valueOf("2016-12-12"))

    assert(
      spark.range(1).map { x => new java.sql.Timestamp(100000) }.head() ==
  ...
  test("SPARK-21567: Dataset should work with type alias") {
    checkDataset(Seq(1).toDS().map(_ => ("", TestForTypeAlias.tupleTypeAlias)), ("", (1, 1)))
  ...
  test("Check RelationalGroupedDataset toString: over length schema ") {
    val kvDataset = (1 to 3)
      .map(x => (x, x.toString, x.toLong))
      .toDF("id", "val1", "val2")
      .groupBy("id")
    val expected = "RelationalGroupedDataset:" +
  ...
  test("Check KeyValueGroupedDataset toString: Named KV-pair") {
    val kvDataset = (1 to 3)
      ...
      .toDF("id", "val1")
      .as[DoubleData]
      .groupByKey(x => DoubleData(x.id, x.val1))
    val expected = "KeyValueGroupedDataset:" +
  ...
  test("SPARK-24548: Dataset with tuple encoders should have correct schema") {
    val encoder =
      Encoders.tuple(newStringEncoder, Encoders.tuple(newStringEncoder, newStringEncoder))
  ...
  test("SPARK-26233: serializer should enforce decimal precision and scale") {
    ...
    checkAnswer(
      df.groupBy(col("a")).agg(first(col("b"))),
  ...
  test("SPARK-26706: Fix Cast.mayTruncate for bytes") {
    val thrownException = intercept[AnalysisException] {
      spark
        .range(Long.MaxValue - 10, Long.MaxValue)
        .as[Byte]
        .map(b => b - 1)
        .collect()
    }
  ...
  val dotColumnTestModes =
    Table(("caseSensitive", "colName"), ("true", "field.1"), ("false", "Field.1"))
  ...
  test("SPARK-39783: backticks in error message for candidate column with dots") {
    checkError(
      ...
      condition = "UNRESOLVED_COLUMN.WITH_SUGGESTION",
      sqlState = None,
      parameters = Map("objectName" -> "`the`.`id`", "proposal" -> "`the.id`"),
      context = ExpectedContext(fragment = "select", getCurrentClassCallSitePattern))
  }
    ...
    checkError(
      exception = intercept[AnalysisException] {
        spark
          ...
          .select($"`map`" ($"nonexisting"))
          .show()
      },
      condition = "UNRESOLVED_MAP_KEY.WITH_SUGGESTION",
      sqlState = None,
      parameters = Map("objectName" -> "`nonexisting`", "proposal" -> "`map`, `other.column`"),
      context = ExpectedContext(fragment = "$", getCurrentClassCallSitePattern))
  }
  ...
  test("groupBy.as") {
    ...
    val exchanges = collect(df3.queryExecution.executedPlan) { case h: ShuffleExchangeExec =>
      h
    }
  ...
  test("SPARK-33390: Make Literal support char array") {
    ...
    checkAnswer(df.where($"zoo".contains(Array('a', 'b'))), Seq(Row("abc")))
  }
  ...
  test(
    "SPARK-35652: joinWith on two table generated from same one performing a cartesian join," +
    ...
    checkDataset(joined, (1, 1), (2, 2), (3, 3))
  }
  ...
  test("SPARK-47385: Tuple encoder with Option inputs") {
    ...
    val input = Seq((SingleData(1), Some(SingleData(1))), (SingleData(2), None))
  ...
  test("CLASS_UNSUPPORTED_BY_MAP_OBJECTS when creating dataset") {
    withSQLConf(
      ...
      implicit val im: ExpressionEncoder[Array[Int]] = ExpressionEncoder(
        AgnosticEncoders
          .IterableEncoder(ClassTag(classOf[Array[Int]]), BoxedIntEncoder, false, false))
      ...
      val expressions = im
        .resolveAndBind(df.queryExecution.logical.output, spark.sessionState.analyzer)
        .createDeserializer()
        .expressions
  ...
  test("SPARK-46791: Dataset with set field") {
    ...
    checkDataset(
      ds.map(t => t),
      WithSet(0, HashSet("foo", "bar")),
      WithSet(1, HashSet("bar", "zoo")))
  }
...
class DatasetLargeResultCollectingSuite extends QueryTest with SharedSparkSession {
...
/**
 * A class used to test serialization using encoders. This class throws exceptions when using Java
 * serialization -- so the only way it can be "serialized" is through our encoders.
 */
