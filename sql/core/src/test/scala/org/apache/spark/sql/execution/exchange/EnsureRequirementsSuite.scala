...
class EnsureRequirementsSuite extends SharedSparkSession {
  ...
  test("reorder should handle KeyGroupedPartitioning") {
    ...
    val plan1 = DummySparkPlan(outputPartitioning =
      KeyGroupedPartitioning(Seq(years(exprA), bucket(4, exprB), days(exprC)), 4))
    val plan2 = DummySparkPlan(outputPartitioning =
  ...
  test("Check with KeyGroupedPartitioning") {
    ...
    var plan1 =
      DummySparkPlan(outputPartitioning = KeyGroupedPartitioning(exprA :: exprB :: Nil, 5))
    var plan2 =
      DummySparkPlan(outputPartitioning = KeyGroupedPartitioning(exprA :: exprC :: Nil, 5))
    var smjExec =
  ...
  test(s"KeyGroupedPartitioning with ${REQUIRE_ALL_CLUSTER_KEYS_FOR_CO_PARTITION.key} = false") {
    var plan1 = DummySparkPlan(outputPartitioning =
      KeyGroupedPartitioning(bucket(4, exprB) :: years(exprC) :: Nil, 4))
    var plan2 = DummySparkPlan(outputPartitioning =
  ...
  test("SPARK-41413: check compatibility when partition values mismatch") {
    withSQLConf(SQLConf.V2_BUCKETING_PUSH_PART_VALUES_ENABLED.key -> "true") {
      ...
      var plan1 = DummySparkPlan(outputPartitioning = KeyGroupedPartitioning(
        bucket(4, exprB) :: bucket(8, exprC) :: Nil,
        leftPartValues.length,
        leftPartValues))
      var plan2 = DummySparkPlan(outputPartitioning = KeyGroupedPartitioning(
  ...
  test("SPARK-42168: FlatMapCoGroupInPandas and Window function with differing key order") {
    ...
    val flapMapCoGroup = FlatMapCoGroupsInPandasExec(
      Seq(Seq(lKey, lKey2), Seq(rKey, rKey2)),
      pythonUdf,
    ...
    result match {
      case FlatMapCoGroupsInPandasExec(
            Seq(leftKeys, rightKeys),
            _,
            _,
            Seq(SortExec(leftOrder, false, _, _), SortExec(rightOrder, false, _, _))) =>
