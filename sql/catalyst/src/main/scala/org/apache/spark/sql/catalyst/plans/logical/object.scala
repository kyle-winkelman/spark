...
object CoGroup {
  def apply[K: Encoder, V: Encoder, U: Encoder, OUT: Encoder](
      func: (K, Iterator[V], Iterator[U]) => IterableOnce[OUT],
      groups: Seq[Seq[Attribute]],
      attrs: Seq[Seq[Attribute]],
    ...
    val f = (key: K, iterators: Seq[Iterator[_]]) =>
      func(key, iterators(0).asInstanceOf[Iterator[V]], iterators(1).asInstanceOf[Iterator[U]])
    val cogrouped = CoGroup(
      f.asInstanceOf[(Any, Seq[Iterator[Any]]) => IterableOnce[Any]],
      // The groups are guaranteed to be of same schema, so it's safe to resolve the
      // `keyDeserializer` based on any of them, here we pick the head one.
      UnresolvedDeserializer(encoderFor[K].deserializer, groups.head),
      Seq(
        UnresolvedDeserializer(encoderFor[V].deserializer, attrs(0)),
        UnresolvedDeserializer(encoderFor[U].deserializer, attrs(1))),
      groups,
      attrs,
      orders,
  ...
  def apply[K: Encoder, V: Encoder, U1: Encoder, U2: Encoder, OUT: Encoder](
      func: (K, Iterator[V], Iterator[U1], Iterator[U2]) => IterableOnce[OUT],
      groups: Seq[Seq[Attribute]],
      attrs: Seq[Seq[Attribute]],
      orders: Seq[Seq[SortOrder]],
      children: Seq[LogicalPlan]): LogicalPlan = {
    ...
    val f = (key: K, iterators: Seq[Iterator[_]]) =>
      func(
        key,
        iterators(0).asInstanceOf[Iterator[V]],
        iterators(1).asInstanceOf[Iterator[U1]],
        iterators(2).asInstanceOf[Iterator[U2]])
    val cogrouped = CoGroup(
      f.asInstanceOf[(Any, Seq[Iterator[Any]]) => IterableOnce[Any]],
      // The groups are guaranteed to be of same schema, so it's safe to resolve the
      // `keyDeserializer` based on any of them, here we pick the head one.
      ...
      Seq(
        UnresolvedDeserializer(encoderFor[V].deserializer, attrs(0)),
        UnresolvedDeserializer(encoderFor[U1].deserializer, attrs(1)),
        UnresolvedDeserializer(encoderFor[U2].deserializer, attrs(2))),
      groups,
      attrs,
      orders,
      CatalystSerde.generateObjAttr[OUT],
  ...
  def apply[K: Encoder, V: Encoder, U1: Encoder, U2: Encoder, U3: Encoder, OUT: Encoder](
      func: (K, Iterator[V], Iterator[U1], Iterator[U2], Iterator[U3]) => IterableOnce[OUT],
      groups: Seq[Seq[Attribute]],
      attrs: Seq[Seq[Attribute]],
      orders: Seq[Seq[SortOrder]],
      children: Seq[LogicalPlan]): LogicalPlan = {
    ...
    val f = (key: K, iterators: Seq[Iterator[_]]) =>
      func(
        ...
        iterators(1).asInstanceOf[Iterator[U1]],
        iterators(2).asInstanceOf[Iterator[U2]],
        iterators(3).asInstanceOf[Iterator[U3]])
    val cogrouped = CoGroup(
      f.asInstanceOf[(Any, Seq[Iterator[Any]]) => IterableOnce[Any]],
      // The groups are guaranteed to be of same schema, so it's safe to resolve the
      // `keyDeserializer` based on any of them, here we pick the head one.
      ...
      Seq(
        ...
        UnresolvedDeserializer(encoderFor[U1].deserializer, attrs(1)),
        UnresolvedDeserializer(encoderFor[U2].deserializer, attrs(2)),
        UnresolvedDeserializer(encoderFor[U3].deserializer, attrs(3))),
      groups,
      attrs,
      orders,
      CatalystSerde.generateObjAttr[OUT],
  ...
  def apply[
    ...
    val f = (key: K, iterators: Seq[Iterator[_]]) =>
      func(
        ...
        iterators(2).asInstanceOf[Iterator[U2]],
        iterators(3).asInstanceOf[Iterator[U3]],
        iterators(4).asInstanceOf[Iterator[U4]])
    val cogrouped = CoGroup(
      f.asInstanceOf[(Any, Seq[Iterator[Any]]) => IterableOnce[Any]],
      // The groups are guaranteed to be of same schema, so it's safe to resolve the
      // `keyDeserializer` based on any of them, here we pick the head one.
      ...
      Seq(
        ...
        UnresolvedDeserializer(encoderFor[U2].deserializer, attrs(2)),
        UnresolvedDeserializer(encoderFor[U3].deserializer, attrs(3)),
        UnresolvedDeserializer(encoderFor[U4].deserializer, attrs(4))),
      groups,
      attrs,
      orders,
      CatalystSerde.generateObjAttr[OUT],
  ...
  def apply[
    ...
    val f = (key: K, iterators: Seq[Iterator[_]]) =>
      func(
        ...
        iterators(3).asInstanceOf[Iterator[U3]],
        iterators(4).asInstanceOf[Iterator[U4]],
        iterators(5).asInstanceOf[Iterator[U5]])
    val cogrouped = CoGroup(
      f.asInstanceOf[(Any, Seq[Iterator[Any]]) => IterableOnce[Any]],
      // The groups are guaranteed to be of same schema, so it's safe to resolve the
      // `keyDeserializer` based on any of them, here we pick the head one.
      ...
      Seq(
        ...
        UnresolvedDeserializer(encoderFor[U3].deserializer, attrs(3)),
        UnresolvedDeserializer(encoderFor[U4].deserializer, attrs(4)),
        UnresolvedDeserializer(encoderFor[U5].deserializer, attrs(5))),
      groups,
      attrs,
      orders,
      CatalystSerde.generateObjAttr[OUT],
  ...
  def apply[
    ...
    val f = (key: K, iterators: Seq[Iterator[_]]) =>
      func(
        ...
        iterators(4).asInstanceOf[Iterator[U4]],
        iterators(5).asInstanceOf[Iterator[U5]],
        iterators(6).asInstanceOf[Iterator[U6]])
    val cogrouped = CoGroup(
      f.asInstanceOf[(Any, Seq[Iterator[Any]]) => IterableOnce[Any]],
      // The groups are guaranteed to be of same schema, so it's safe to resolve the
      // `keyDeserializer` based on any of them, here we pick the head one.
      ...
      Seq(
        ...
        UnresolvedDeserializer(encoderFor[U4].deserializer, attrs(4)),
        UnresolvedDeserializer(encoderFor[U5].deserializer, attrs(5)),
        UnresolvedDeserializer(encoderFor[U6].deserializer, attrs(6))),
      groups,
      attrs,
      orders,
      CatalystSerde.generateObjAttr[OUT],
...
/**
 * A relation produced by applying `func` to each grouping key and associated values from
 * children.
 */
case class CoGroup(
